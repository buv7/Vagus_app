...
import '../../components/workout/exercise_search_filter_sheet.dart';
...
class _CoachPlanBuilderScreenState extends State<CoachPlanBuilderScreen> {
  ...
  final Map<String, List<MusicLink>> _musicLinksByDay = {}; // weekIndex_dayIndex -> [MusicLink]
  ...
  void addWeek() {
    setState(() {
      weeks.add({'days': [], 'label': '', 'tags': <String>[]});
    });
  }
  ...
  Future<void> _showAISuggestions(int weekIndex, int dayIndex, int exIndex) async {
  ...
  Future<void> _showExerciseHistory(int weekIndex, int dayIndex, int exIndex) async {
  ...
  // Week metadata editor (label + tags)
  void _editWeekMetadata(int weekIndex) {
    final current = Map<String, dynamic>.from(weeks[weekIndex]);
    final labelController = TextEditingController(text: (current['label'] ?? '').toString());
    final tagsController = TextEditingController(text: List<String>.from((current['tags'] as List?)?.map((e) => e.toString()) ?? const []).join(', '));

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Edit Week Label/Tags'),
        content: Column(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final tags = tagsController.text
                  ...
                  .toList();
              setState(() {
                weeks[weekIndex]['label'] = label;
                weeks[weekIndex]['tags'] = tags;
              });
              Navigator.pop(context);
            },
            child: const Text('Save'),
  ...
  Widget build(BuildContext context) {
    ...
    final int totalWeeks = weeks.length;
    int totalDays = 0;
        ...
        actions: [
          if (weeks.isNotEmpty) // Only show AI actions if there's a plan
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                const PopupMenuItem(
                  value: 'suggest_progression',
                  child: Row(
                    children: [
                      Icon(Icons.trending_up),
                      SizedBox(width: 8),
                      Text('Suggest Progression'),
                    ],
                  ),
                ),
                const PopupMenuItem(
                ...
                const PopupMenuItem(
                  value: 'weak_point_analysis',
                  child: Row(
                    children: [
                      Icon(Icons.analytics),
                      SizedBox(width: 8),
                      Text('Weak Point Analysis'),
                    ],
                  ),
                ),
              ],
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              tooltip: 'Edit week label/tags',
                              onPressed: () => _editWeekMetadata(weekIndex),
                            ),
                            IconButton(
                                  ...
                                  children: [
                                                      ...
                                                      child: Row(
                                                        children: [
                                                          ...
                                                          IconButton(
                                                            ...
                                                            onPressed: () => unawaited(_showAISuggestions(weekIndex, dayIndex, exIndex)),
                                                          ),
                                                          // NEW: Exercise history button
                                                          IconButton(
                                                            tooltip: 'Exercise History',
                                                            icon: const Icon(Icons.history),
                                                            onPressed: () => unawaited(_showExerciseHistory(weekIndex, dayIndex, exIndex)),
                                                          ),
                                    ...
                                    Align(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          IconButton(
                                            tooltip: 'Filter & search exercises',
                                            icon: const Icon(Icons.filter_list),
                                            onPressed: () async {
                                              final selected = await showModalBottomSheet<String>(
                                                context: context,
                                                isScrollControlled: true,
                                                builder: (ctx) => const ExerciseSearchFilterSheet(),
                                              );
                                              if (selected != null && selected.isNotEmpty) {
                                                addExercise(weekIndex, dayIndex);
                                                final exIdx = (weeks[weekIndex]['days'][dayIndex]['exercises'] as List).length - 1;
                                                updateExercise(weekIndex, dayIndex, exIdx, 'name', selected);
                                              }
                                            },
                                          ),
                                          const SizedBox(width: 4),
                                    ...
                                    /// Music Attachment
                                    MusicAttachmentWidget(
                                      planId: clientId.isNotEmpty ? 'temp_plan_id' : null, // Will be set when plan is saved
                                      weekIdx: weekIndex,
                                      dayIdx: dayIndex,
                                      attachedLinks: _musicLinksByDay['${weekIndex}_$dayIndex'] ?? [],
                                      onLinksChanged: (links) {
                                        setState(() {
                                          _musicLinksByDay['${weekIndex}_$dayIndex'] = links;
                                        });
                                      },
                                      isReadOnly: false,
                                    ),
  ...
  Future<void> _handleAIAction(String action) async {
    if (weeks.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add some weeks to the plan first')),
      );
      return;
    }
    ...
    try {
      // Serialize current plan to JSON
      final planJson = json.encode({
        'name': planName,
      ...
      switch (action) {
        case 'suggest_progression':
          result = await WorkoutAI.suggestProgression(planJson: planJson);
          break;
        case 'deload_advice':
          result = await WorkoutAI.deloadAdvice(planJson: planJson);
          break;
        case 'weak_point_analysis':
          result = await WorkoutAI.weakPointAnalysis(planJson: planJson);
          break;
        default:
      ...
      _showAIResultSheet(action, result);
    } catch (e) {
      // Close loading dialog
      if (!mounted || !context.mounted) return;
      Navigator.pop(context);
  ...
  void _showAIResultSheet(String action, String result) {
    final actionTitles = {
      'suggest_progression': 'Progression Suggestions',
      'deload_advice': 'Deload Advice',
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    actionTitles[action] ?? 'AI Analysis',
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () {
                        Navigator.pop(context);
                        // TODO: Add to notes functionality
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Add to notes - coming soon')),
                        );
                      },
