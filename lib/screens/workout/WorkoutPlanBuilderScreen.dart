...
class _WorkoutPlanViewerScreenState extends State<WorkoutPlanViewerScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        if (_plans.isNotEmpty) {
          _plan = _plans.first;
          _selectedClientId = _plan!['client_id']?.toString();
        }
        _loading = false;
      });

      print("📦 Loaded plans: ${_plans.map((p) => p['id']).toList()}");
      print("✅ Current selected: ${_plan?['id']}");
    } catch (e) {
      setState(() {
  ...
  Future<void> _populatePlansForContext() async {
    try {
      ...
      print("✅ Loaded ${_plans.length} plans for dropdown");
    } catch (e) {
      print("❌ Error loading plans for dropdown: $e");
    }
  }
...
class AttachmentViewer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (type.startsWith('image')) {
      ...
      return Row(children: const [Icon(Icons.picture_as_pdf), Text('PDF')]);
    } else {
      return Row(children: const [Icon(Icons.insert_drive_file), Text('File')]);
    }
  }
}
