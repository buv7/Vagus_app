        ...
        children: [
          ...
          Row(
            children: [
              if (last?.weight != null) ...[
                OutlinedButton(
                  onPressed: () => _applyLast(last!.weight!, theme),
                  style: OutlinedButton.styleFrom(
                    foregroundColor: theme.colorScheme.primary,
                    side: BorderSide(color: theme.colorScheme.primary.withValues(alpha: 0.5)),
                  ),
                  child: Text('Apply last (${last!.weight!.toStringAsFixed(0)} ${useKg ? 'kg' : 'lb'})'),
                ),
                const SizedBox(width: 8),
              ],
              if (advice.delta != null && last?.weight != null) ...[
                FilledButton(
                  onPressed: () => _applySuggested(last!.weight! + advice.delta!, theme),
                  child: Text('Apply suggested (${(last!.weight! + advice.delta!).toStringAsFixed(0)} ${useKg ? 'kg' : 'lb'})'),
                ),
