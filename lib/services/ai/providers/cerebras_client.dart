...
class CerebrasClient implements ProviderClient {
  ...
  CerebrasClient() : _apiKey = const String.fromEnvironment('CEREBRAS_API_KEY');

  @override
  String get providerId => 'cerebras';
  ...
  Future<String> complete(
    ...
    if (response.statusCode != 200) {
      throw Exception('Cerebras ${response.statusCode}: ${response.body}');
    }
  ...
  Future<String> vision(Uint8List imageBytes, String prompt) =>
      throw UnsupportedError('Cerebras does not support vision');

  @override
  Future<List<double>> embed(String input) =>
      throw UnsupportedError('Cerebras does not support embeddings');
  ...
  void _assertKey() {
    if (_apiKey.isEmpty) {
      throw StateError(
          'CEREBRAS_API_KEY is not set. Pass via --dart-define=CEREBRAS_API_KEY=<key>.');
    }
  }
