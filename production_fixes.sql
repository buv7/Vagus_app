-- ========================================
-- VAGUS APP PRODUCTION FIXES
-- ========================================
-- This file contains all the database fixes needed for production
...
DO $$
BEGIN
  -- Users can read their own coach-client relationships
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'user_coach_links_select_own') THEN
    CREATE POLICY user_coach_links_select_own ON public.user_coach_links
      FOR SELECT TO authenticated
      USING (client_id = auth.uid() OR coach_id = auth.uid());
  END IF;

  -- Coaches can create relationships with clients
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'user_coach_links_insert_coach') THEN
    CREATE POLICY user_coach_links_insert_coach ON public.user_coach_links
      FOR INSERT TO authenticated
      WITH CHECK (coach_id = auth.uid());
  END IF;

  -- Coaches can delete relationships with their clients
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'user_coach_links_delete_coach') THEN
...
-- ========================================
-- Drop existing coach_clients table if it exists, then create view
...
DO $$
BEGIN
  -- Users can read events they created or are involved in
