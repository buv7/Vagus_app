-- Migration: Update Search to Include Muscle Aliases
-- Phase: Multilingual Knowledge Expansion
-- Date: 2025-01-22
--
-- This migration updates the search_exercises_with_aliases function
-- to include muscle_aliases table for better Arabic muscle search.
--
-- Search now matches:
-- - English muscle keys
-- - Arabic muscle names (from muscle_translations)
-- - Arabic muscle aliases (from muscle_aliases) - NEW
-- - Full-text search on Arabic muscle aliases - NEW
...
BEGIN
  ...
  IF p_muscles IS NOT NULL AND p_muscles != '{}' THEN
    ...
    IF EXISTS (
      ...
      -- First try muscle_translations (existing)
      ...
      -- Also try muscle_aliases (NEW)
      SELECT ARRAY_AGG(DISTINCT ma.muscle_key)
      INTO v_query_muscle_keys
      FROM muscle_aliases ma
      WHERE ma.language = 'ar'
        AND (
          ma.alias = ANY(p_muscles)
          OR ma.alias ILIKE ANY(SELECT '%' || m || '%' FROM unnest(p_muscles) m)
          OR to_tsvector('arabic', ma.alias) @@ plainto_tsquery('arabic', array_to_string(p_muscles, ' '))
        );
      
      -- Combine resolved keys
      v_muscle_keys := ARRAY(
        SELECT DISTINCT unnest(
          COALESCE(v_muscle_keys, ARRAY[]::TEXT[]) || 
          COALESCE(v_query_muscle_keys, ARRAY[]::TEXT[]) || 
          p_muscles
        )
      );
    ELSE
  ...
  WHERE
    ...
    -- Muscle filter (array overlap) - now supports Arabic muscle names and aliases
    AND (
      ...
      -- Match Arabic muscle names in primary muscles (from muscle_translations)
      OR EXISTS (
        SELECT 1
      ...
      -- Match Arabic muscle names in secondary muscles (from muscle_translations)
      OR EXISTS (
        SELECT 1
      ...
      -- Match Arabic muscle aliases in primary muscles (from muscle_aliases) - NEW
      OR EXISTS (
        SELECT 1
        FROM unnest(ek.primary_muscles) pm
        JOIN muscle_aliases ma ON ma.muscle_key = pm AND ma.language = 'ar'
        WHERE ma.alias = ANY(v_muscle_keys)
           OR ma.alias ILIKE ANY(SELECT '%' || m || '%' FROM unnest(v_muscle_keys) m)
      )
      -- Match Arabic muscle aliases in secondary muscles (from muscle_aliases) - NEW
      OR EXISTS (
        SELECT 1
        FROM unnest(ek.secondary_muscles) sm
        JOIN muscle_aliases ma ON ma.muscle_key = sm AND ma.language = 'ar'
        WHERE ma.alias = ANY(v_muscle_keys)
           OR ma.alias ILIKE ANY(SELECT '%' || m || '%' FROM unnest(v_muscle_keys) m)
      )
    )
    ...
    -- OR Arabic muscle names (from muscle_translations)
    -- OR Arabic muscle aliases (from muscle_aliases) - NEW
    AND (
      p_query IS NULL 
      OR p_query = ''
      -- English search
      ...
      -- Arabic muscle search (from muscle_translations - existing)
      OR EXISTS (
        SELECT 1
      ...
      -- Arabic muscle aliases search (from muscle_aliases) - NEW
      OR EXISTS (
        SELECT 1
        FROM unnest(ek.primary_muscles || ek.secondary_muscles) muscle_key
        JOIN muscle_aliases ma ON ma.muscle_key = muscle_key AND ma.language = 'ar'
        WHERE 
          ma.alias ILIKE '%' || p_query || '%'
          OR to_tsvector('arabic', ma.alias) @@ plainto_tsquery('arabic', p_query)
      )
    )
  ORDER BY 
    -- Prioritize exact name matches, then alias matches, then partial matches
    -- Now includes Arabic matches (exercises and muscles, including muscle aliases)
    CASE 
      ...
      -- Arabic muscle name exact match (from muscle_translations)
      WHEN p_query IS NOT NULL AND EXISTS (
        SELECT 1
        FROM unnest(ek.primary_muscles || ek.secondary_muscles) muscle_key
        JOIN muscle_translations mt ON mt.muscle_key = muscle_key AND mt.language = 'ar'
      ...
      -- Arabic muscle alias exact match (from muscle_aliases) - NEW
      WHEN p_query IS NOT NULL AND EXISTS (
        SELECT 1
        FROM unnest(ek.primary_muscles || ek.secondary_muscles) muscle_key
        JOIN muscle_aliases ma ON ma.muscle_key = muscle_key AND ma.language = 'ar'
        WHERE ma.alias ILIKE p_query
      ) THEN 3
      -- Partial matches
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration complete: update_search_with_muscle_aliases';
  RAISE NOTICE '   - Updated function: search_exercises_with_aliases';
  RAISE NOTICE '   - Function now includes muscle_aliases table for Arabic search';
  RAISE NOTICE '   - Muscle filters now support Arabic muscle aliases';
  RAISE NOTICE '   - Full-text search on Arabic muscle aliases enabled';
